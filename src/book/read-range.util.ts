...
export function mergeReadRanges(existing: ReadRange[], newRanges: ReadRange[]): ReadRange[] {
  let result = existing;
  for (const nr of newRanges) result = mergeReadRange(result, nr);
  return result;
}

/** ranges 내 고유 GI 총 개수 (각 구간 길이 합) */
export function sumReadRanges(ranges: ReadRange[]): number {
  let sum = 0;
  for (const r of ranges) sum += r.to - r.from + 1;
  return sum;
}

/** 개별 GI 배열 → 정렬된 ReadRange[] 변환 */
export function gisToRanges(gis: number[]): ReadRange[] {
  if (gis.length === 0) return [];
  const sorted = [...new Set(gis)].sort((a, b) => a - b);
  const ranges: ReadRange[] = [];
  let from = sorted[0], to = sorted[0];
  for (let i = 1; i < sorted.length; i++) {
    if (sorted[i] === to + 1) to = sorted[i];
    else { ranges.push({ from, to }); from = sorted[i]; to = sorted[i]; }
  }
  ranges.push({ from, to });
  return ranges;
}
...
export function filterDwellToRanges(
  gazeDwellMap: Record<string, number>,
  thresholdMs: number,
): ReadRange[] {
  const passedGIs: number[] = [];
  for (const [gi, ms] of Object.entries(gazeDwellMap)) {
    if (ms >= thresholdMs) passedGIs.push(Number(gi));
  }
